...
export default function HQDashboard({ onViewStore }) {
  ...
  const card          = { background: '#fff', border: '1px solid #EDE0CC', borderRadius: 12, padding: 16 }
  ...
  return (
    <div style={{ maxWidth: 960 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 28 }}>
        {[
          { label: 'Total Stores',     value: activeStores.length,  color: '#C8843A', sub: pendingStores.length ? `${pendingStores.length} pending` : 'all active' },
          ...
          { label: 'Open Invitations', value: invitations.length,   color: invitations.length > 0 ? '#2980B9' : '#8B7355', sub: invitations.length > 0 ? 'awaiting sign-up' : 'none pending' },
      ...
      <div style={{ ...card, padding: 0, overflow: 'hidden', marginBottom: 28 }}>
        {activeStores.length === 0 ? (
          <div style={{ padding: 32, textAlign: 'center', color: '#8B7355', fontSize: 13 }}>No stores yet.</div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#FAFAF8', borderBottom: '2px solid #EDE0CC' }}>
                  {['Store', 'Revenue (this month)', 'Last Checklist', 'Low Stock', 'Users', ''].map((h, i) => (
                    <th key={i} style={{ padding: '11px 16px', textAlign: i === 0 ? 'left' : 'center', color: '#8B7355', fontWeight: 600, fontSize: 12, whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {activeStores.map(store => {
                  ...
                  return (
                    <tr key={store.id} style={{ borderTop: '1px solid #EDE0CC' }}>
                      <td style={{ padding: '13px 16px' }}>
                        <div style={{ fontWeight: 700, color: '#2C1810', fontSize: 14 }}>{store.name}</div>
                        {store.address && <div style={{ fontSize: 11, color: '#8B7355', marginTop: 1 }}>{store.address}</div>}
                      </td>
              ...
              {!loadingMet && activeStores.length > 1 && (
                <tfoot>
                  <tr style={{ borderTop: '2px solid #EDE0CC', background: '#FAFAF8' }}>
                    <td style={{ padding: '11px 16px', fontWeight: 700, color: '#2C1810', fontSize: 13 }}>Total</td>
